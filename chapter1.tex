...
CAM, version 6 (CAM6; \url{https://ncar.github.io/CAM/doc/build/html/users_guide/index.html}) contains a replacement of the stratiform scheme, planetary boundary layer scheme and shallow convection scheme with a unified assumed probability distribution function scheme referred to as the Cloud Layer Unified by Binormals \citep{GETAL2002JAS}. The gravity wave drag scheme has been replaced with an anisotropic gravity wave drag scheme that uses the orientation of topographic ridges to determine the direction of propagation.
...
Chapter~\ref{sec:chapter6} contains a convergence study of CAM6 using the CAM-SE dynamical core, with a new dry mass vertical coordinate \citep{LetAl2018JAMES}, and the Conservative Semi-Lagrangian Advection Method for tracer advection \citep[CSLAM; ][]{LTOUNGK2017MWR}, in which the physics are evaluated on the CSLAM grid \citep{HL2018MWR}. Table~\ref{tbl:table1-6} shows the results of a convergence study in an aqua-planet configuration \citep[`QOBS' SST profile in][]{NH2000ASL}.
